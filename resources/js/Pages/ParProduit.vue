    ...
    <div class="flex-1 bg-bg p-5 overflow-y-auto">
        ...
        <div class="flex items-center justify-between mb-4">
            <h2 class="font-mono text-[11px] font-bold text-navy tracking-[2px] uppercase">
                📦 TCO par Famille de Produit
            </h2>
            <div class="flex items-center gap-2">
                <label class="text-[10px] text-muted font-medium">Trier par :</label>
                <select
                    v-model="sortBy"
                    class="bg-white border border-border rounded-md text-text font-mono text-[11px] py-1 px-2 focus:border-accent outline-none"
                >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            >
                ...
                <div
                    ...
                    :style="{ backgroundColor: familleColor(group.famille) + '15', borderBottom: '2px solid ' + familleColor(group.famille) }"
                >
                    <span
                        class="font-mono font-bold text-sm"
                        :style="{ color: familleColor(group.famille) }"
                    >
                        {{ group.famille }}
                    </span>
                    <span
                        class="text-[10px] font-mono px-2 py-0.5 rounded"
                        :style="{ backgroundColor: familleColor(group.famille) + '20', color: familleColor(group.famille) }"
                    >
                    ...
                    <div>
                        ...
                        <div class="h-2 bg-[#f0f4f9] rounded overflow-hidden">
                            <div
                                ...
                                :style="{ width: group.pct_tco + '%', backgroundColor: familleColor(group.famille) }"
                            ></div>
                        </div>
                    </div>
            ...
            >
                Aucune donnée à afficher. Ajoutez des dossiers pour voir les statistiques par produit.
            </div>
        </div>
    </div>
</template>
